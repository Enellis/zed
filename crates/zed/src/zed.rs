...
pub fn handle_keymap_file_changes(
    ...
    ModalEditorFlavourSetting::register(cx);

    let (base_keymap_tx, mut base_keymap_rx) = mpsc::unbounded();
    let mut old_base_keymap = *BaseKeymap::get_global(cx);
    let mut old_modal_editor_method = *ModalEditorFlavourSetting::get_global(cx);
    cx.observe_global::<SettingsStore>(move |cx| {
        let new_base_keymap = *BaseKeymap::get_global(cx);
        let new_modal_editor_method = *ModalEditorFlavourSetting::get_global(cx);
...
pub fn load_default_keymap(cx: &mut AppContext) {
    ...
    match *ModalEditorFlavourSetting::get_global(cx) {
        ModalEditorFlavourSetting::Vim => KeymapFile::load_asset("keymaps/vim.json", cx).unwrap(),
        _ => (),
    }
